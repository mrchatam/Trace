...
- todo_ids: [P37-S01-01]
- role: implementer
- skills: [planning-and-task-breakdown, spec-driven-development, source-driven-development]
- verification: automated

## Objective

Author **only** `scopes/scope-01-plan/PLAN.md` from the locked content below (S01-00). Copy §1–§8 into PLAN.md with any live-repo path tweaks noted in Notes. **No product code.**

## References

- [docs/rules/agent-loop-protocol.md](../../../../rules/agent-loop-protocol.md)
- [RESIDUALS.md](../scope-00-triage/RESIDUALS.md) (S00-02 APPROVE)
- [DESIGN-LOCKS.md](../../DESIGN-LOCKS.md)
- Phase 36 [`PLAN.md`](../../../phase-36-gate-honesty-terminal-tasks/scopes/scope-01-plan/PLAN.md) (structure + touch-list pattern)

## Session start

Follow agent-loop-protocol Session start. Block if RESIDUALS.md missing or S00-02 not APPROVE.

## Locked defaults (S01-00 — do not re-debate)

| Item | Value |
|------|-------|
| Artifact | `PLAN.md` in this scope folder |
| Accept source | RESIDUALS.md §2 accept rows only — no scope creep |
| S02 scope | **8 accepts:** R1–R6, R8, R11 |
| S03 scope | R10 accept (browser verify); R9 re-defer (dogfood doc) |
| Touch-list order | **library → MCP → HTTP → install → GUI** (Law 19) |
| R1 | Advisory-only — **never** set `PlanExists=true` or write planner rows |
| R7 | Re-defer — default enforce stays **`EnforceOff`** when config missing |
| R2 minimum | `POST /v1/plans/bootstrap` only; create-coarse/deep POST **optional** if PLAN § notes expansion |
| R11 | Doc/workflow path only — **no** new MCP `critique-seed` tool |
| Regression | PLAN § VERIFY must include Phase 36 acceptance subset (Block 0) |

---

## §1 Summary (for PLAN.md)

| Metric | Value |
|--------|------:|
| **Accept (S02)** | 8 — R1–R6, R8, R11 |
| **Accept (S03)** | 1 — R10 live GUI verify |
| **Re-defer** | 2 — R7, R9 (+ R8 full plan screen in §6 registry) |
...
## §3 Re-deferred residuals (PLAN §3)
...
### Permanent rejects (document in PLAN §3)
...
## §5 Acceptance tests (S02 implements — name in PLAN)
...
Include in PLAN § VERIFY Block 0:
...
1. Read RESIDUALS.md + this file locked §1–§8.
2. Author `PLAN.md` copying structure §1–§8 (adjust headings to match P36 PLAN style).
3. Verify every S02 accept row (R1–R6, R8, R11) has test hook in §5.
4. Verify R1 explicitly forbids silent PlanExists in §2 and §7.
5. Verify R7 re-defer + enforce default **off** in §3.
6. Update board row **639** → `done` with Notes listing accept IDs.

## Exit criteria

- [ ] `PLAN.md` complete §1–§8 mirroring locked content above
- [ ] Every RESIDUALS S02 accept row in PLAN §2 with test hook in §5
- [ ] Touch-list order library → MCP → HTTP → install → GUI in PLAN §4
- [ ] VERIFY mapping §6 present for S03-00
- [ ] R1 forbids silent PlanExists; R7 default off locked
- [ ] Board row P37-S01-01 → `done` with Notes (accept IDs: R1–R6,R8,R11)

## Minimal todos

- [ ] Copy §1–§8 into `PLAN.md` (P36 PLAN.md formatting)
- [ ] Spot-check live paths (`apply.go`, `tools_loop.go`, `server.go`, `Overview.tsx`)
- [ ] Confirm Phase 36 regression list in PLAN § VERIFY

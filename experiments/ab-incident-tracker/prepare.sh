#!/usr/bin/env bash
# Prepare E01 workspaces. Never wipe an arm unless that arm is requested.
#
...
b0_has_product_work() {
  ...
  [[ -d "$ws" ]] || return 1
  # Starter is a single empty main.go. Extra Go/HTML/docs mean B0 already ran.
  local extra
...
prep_ws() {
  ...
  (
    ...
    git add -A
    git commit -q -m "init $arm starter (E01 incidentops)"
  )
}
...
install_workspace_guard() {
  local ws="$1"
  local rule_src="$EXP/.cursor/rules/e01-workspace-only.mdc"
  if [[ -f "$rule_src" ]]; then
    mkdir -p "$ws/.cursor/rules"
    cp -a "$rule_src" "$ws/.cursor/rules/e01-workspace-only.mdc"
  fi
}
...
echo "prepared E01: target=$TARGET TRACE_BIN=$TRACE_BIN"

if [[ "$do_g1" -eq 1 ]]; then
  WS="$EXP/runs/G1"
  TASK="e0100000-0000-4000-8000-000000000010"
  if ! "$TRACE_BIN" -C "$WS" loop gate --task "$TASK" --for edit >/tmp/e01-gate.json 2>/dev/null; then
    if grep -q plan_missing /tmp/e01-gate.json 2>/dev/null; then
      echo "verify: gate blocks edit on fresh seed (plan_missing) — OK"
    else
      echo "warning: gate blocked but reason unexpected — see /tmp/e01-gate.json" >&2
    fi
  else

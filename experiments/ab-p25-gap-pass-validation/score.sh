#!/usr/bin/env bash
# E02 ab-p25-gap-pass-validation — harness scoring + Phase 25 checks.
...
if [[ "$ARM" == "B0" ]]; then
  ...
  if [[ -f "$WS/cmd/checkoutd/main.go" ]] && [[ $(find "$WS/internal" -name '*.go' 2>/dev/null | wc -l) -gt 0 ]]; then
    pass "B0-2 deliverable beyond starter"
  elif [[ -f "$WS/cmd/checkoutd/main.go" ]] && [[ -f "$WS/SPEC.md" ]]; then

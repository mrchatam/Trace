#!/usr/bin/env bash
# Prepare E02 workspaces. Never wipe an arm unless that arm is requested.
#
...
prep_ws() {
  ...
  (
    ...
    git add -A
    git commit -q -m "init $arm starter (E02 checkout desk)"
  )
}
...
install_workspace_guard() {
  local ws="$1"
  local rule_src="$EXP/.cursor/rules/e02-workspace-only.mdc"
  if [[ -f "$rule_src" ]]; then
    mkdir -p "$ws/.cursor/rules"
    cp -a "$rule_src" "$ws/.cursor/rules/e02-workspace-only.mdc"
  fi
}
...
echo "prepared E02: target=$TARGET TRACE_BIN=$TRACE_BIN"

if [[ "$do_g1" -eq 1 ]]; then
  WS="$EXP/runs/G1"
  RULES="$WS/.cursor/rules/trace-enforcement.mdc"
  if grep -q "mandatory gap pass" "$RULES" 2>/dev/null; then
    echo "verify: Phase 25 GapPassPrompt present in installed rules — OK"
  else
    echo "warning: GapPassPrompt missing — rebuild trace from Phase 25 branch" >&2
  fi
  TASK="e0200000-0000-4000-8000-000000000010"
  if ! "$TRACE_BIN" -C "$WS" loop gate --task "$TASK" --for edit >/tmp/e02-gate.json 2>/dev/null; then
    if grep -q plan_missing /tmp/e02-gate.json 2>/dev/null; then
      echo "verify: gate blocks edit on fresh seed (plan_missing) — OK"
    else
      echo "warning: gate blocked but reason unexpected — see /tmp/e02-gate.json" >&2
    fi
  else

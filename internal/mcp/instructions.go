...
1. **Pick work:** trace_tasks (capped page; default limit 50 — pass cursor/limit/work_state, all=true caps at 500) → trace_context task_id=<uuid> [query=<optional agent query>] (prefer format=json; avoid format=both)
...
Use progressive caps; never request full graph dump; trace_tasks all=true is hard-capped at 500.
